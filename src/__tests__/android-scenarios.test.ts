...
import { 
  AndroidAccessibilityTestHelper
} from './test-utils/android-mocks.helper';
...
const mockNativeModule = ExpoAccessibilityServiceModule as jest.Mocked<typeof ExpoAccessibilityServiceModule>;
...
describe('Android Accessibility Service - Real World Scenarios', () => {
  ...
  describe('Permission Request Scenarios', () => {
    ...
    test('Scenario: SecurityException - app lacks permission to start activity', async () => {
      helper.setupFailedPermissionRequest('SecurityException: Permission denied');
    ...
    test('Scenario: ActivityNotFoundException - accessibility settings not available', async () => {
      helper.setupFailedPermissionRequest('ActivityNotFoundException: No Activity found');
  ...
  describe('Android System Edge Cases', () => {
    test('Scenario: Extremely long services list - performance stress test', async () => {
      ...
      const longServicesList = Array.from({ length: 100 }, (_, i) => 
