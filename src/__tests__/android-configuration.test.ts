...
describe('Android Accessibility Service Configuration Tests', () => {
  ...
  beforeEach(() => {
    ...
    const detectedServices = ['com.example.testaccessibility.MyAccessibilityService'];

    mockModule.setServiceClassName.mockImplementation(async (className: string) => {
    ...
    mockModule.isEnabled.mockImplementation(async () => {
      ...
      if (configuredServiceClassName) {
        ...
        serviceNamesToCheck = detectedServices.map(service => `${packageName}/${service}`);
      } else {
        // Fall back to default
        serviceNamesToCheck = [`${packageName}/${packageName}.MyAccessibilityService`];
      }
      ...
      return serviceNamesToCheck.some(serviceName => serviceList.includes(serviceName));
    });
  });
  ...
  describe('Service Configuration', () => {
    test('should allow setting custom service class name', async () => {
      ...
      expect(mockModule.setServiceClassName).toHaveBeenCalledWith(customServiceName);
    });
    ...
    test('should not detect custom service when different service is enabled', async () => {
      ...
      helper.settings.Secure.getString.mockReturnValue('com.example.testaccessibility/com.example.testaccessibility.DifferentService');
    ...
    test('should support multiple custom service names in configuration workflow', async () => {
      ...
      expect(mockModule.setServiceClassName).toHaveBeenCalledTimes(serviceNames.length);
    });
  });
  ...
  describe('Service Auto-Detection', () => {
    ...
    test('should use auto-detected service when no configuration is set', async () => {
      ...
      const packageName = helper.context.packageName;
      const autoDetectedService = 'com.example.testaccessibility.MyAccessibilityService';
  ...
  describe('Configuration Priority and Fallback', () => {
    test('should prioritize configured service name over auto-detection', async () => {
      ...
      helper.settings.Secure.getString.mockReturnValue(`${packageName}/${configuredService}`);
      
      const result = await isEnabled();
      
      expect(result).toBe(true);
      expect(mockModule.setServiceClassName).toHaveBeenCalledWith(configuredService);
    });
    ...
    test('should not detect auto-detected service when different service is configured', async () => {
      ...
      helper.settings.Secure.getString.mockReturnValue(`${packageName}/${autoDetectedService}`);
    ...
    test('should demonstrate complete configuration workflow', async () => {
      ...
      helper.settings.Secure.getString.mockReturnValue(`${packageName}/${preferredService}`);
      
      const isServiceEnabled = await isEnabled();
      expect(isServiceEnabled).toBe(true);
      
      // Step 4: Verify configuration took precedence
      expect(mockModule.setServiceClassName).toHaveBeenCalledWith(preferredService);
    });
  });
});